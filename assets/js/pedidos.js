...
window.renderTable = () => {
  ...
  tb.innerHTML = fil.map(p => {
    ...
    const tracking = p.tracking ? `<a href="https://www.correosdemexico.gob.mx/SSLServicios/ConsultaCP/Guia.aspx?n=${p.tracking}" target="_blank" style="font-size:11px;color:var(--accent)" title="Rastrear">${p.tracking}</a>` : '—';
    return `<tr>
      ...
      <td><div style="display:flex;gap:6px">
        ...
        <button class="btn-icon" onclick="editTracking('${p.id}','${p.tracking||''}')" title="Agregar guía de rastreo"><i class="bi bi-truck"></i></button>
...
window.guardarTracking = async () => {
  ...
  const histEntry = { accion: `Guía de rastreo: ${num||'(eliminada)'}`, fecha: Date.now() };
...
window.verDetalle = (id) => {
  ...
  document.getElementById('detalle-body').innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
      ...
      ${p.tracking?`<div><div style="font-size:11px;color:var(--text-faint)">Guía de rastreo</div><code style="color:var(--accent)">${p.tracking}</code></div>`:''}
    </div>
