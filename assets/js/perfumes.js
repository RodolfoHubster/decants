...
window.renderTable = () => {
  ...
  tb.innerHTML = fil.map(p => {
    ...
    const tags = Object.entries(pr).filter(([,v]) => +v > 0)
    ...
    return `<tr>
      ...
      <td>
        <button class="btn btn-outline btn-sm" onclick="copiarLista('${p.id}')" title="Copiar lista de precios"><i class="bi bi-clipboard"></i></button>
      </td>
