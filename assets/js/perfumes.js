...
window.renderTable = () => {
  ...
  tb.innerHTML = fil.map(p => {
    ...
    // badge Novedad
    const novBadge = p.novedad
      ? '<span class="badge badge-gold" style="margin-left:6px">✨ Novedad</span>' : '';
    ...
    return `<tr>
      ...
      <td><strong>${p.nombre}</strong>${oculto}${novBadge}</td>
...
window.openModal = () => {
  ...
  document.getElementById('p-novedad').checked   = false;  // ← nuevo
...
window.save = async () => {
  ...
  try {
    ...
    const data = {
      ...
      novedad: document.getElementById('p-novedad').checked   // ← nuevo
    };
    if (id) {
